...
const CONSUMABLE_MARKS =
  /세제|세정제|세척제|린스|린스젤|헹굼제|타블렛|캡슐\s*세제|파우더|액상\s*세제|정제형|살균\s*티슈|탈취제|방향제|섬유유연제|건조\s*시트|향기\s*시트|드라이어\s*시트|섬유\s*시트|랙\b|바구니|트레이(?!닝)|수저통|홀더|칸막이|보관함|정리함|매트\s*시트|스티커|라벨/;
