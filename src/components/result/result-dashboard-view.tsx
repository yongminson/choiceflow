            ...
            <table className="w-full min-w-[min(100%,520px)] border-collapse text-left text-sm">
              ...
              <tbody className="divide-y divide-white/15 text-[13px] leading-relaxed sm:text-sm">
                {Array.from({ length: prosRows }, (_, i) => (
                  <tr key={`pro-${i}`}>
                    <td className="px-4 py-3.5 font-medium text-muted-foreground sm:px-6">장점 {i + 1}</td>
                    <td className="px-4 py-3.5 text-foreground sm:px-6">{m.table?.A?.pros?.[i] ?? "—"}</td>
                    <td className="px-4 py-3.5 text-foreground sm:px-6">{m.table?.B?.pros?.[i] ?? "—"}</td>
                  </tr>
                ))}
                <tr className="bg-rose-500/[0.06] dark:bg-rose-500/[0.08]">
                  ...
                  <td className="px-4 py-3.5 align-top text-foreground sm:px-6">
