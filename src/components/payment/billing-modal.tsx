        ...
        <div className="min-h-0 flex-1 overflow-y-auto px-6 sm:px-8">
          {tab === "credit" ? (
            <div className="pb-4">
              ...
              <div className="mt-3 grid gap-3">
                {CREDIT_PACKS.map((pack) => {
                  ...
                  return (
                    <button key={pack.id} type="button" onClick={() => setSelectedPackId(pack.id)} className={cn("relative flex w-full flex-col rounded-2xl border px-4 py-4 text-left transition-all", "bg-gradient-to-br from-white/[0.06] to-black/20", "hover:border-primary/35 hover:shadow-[0_0_24px_-8px_rgba(99,102,241,0.45)]", selected ? "border-primary/60 ring-2 ring-primary/30 shadow-[0_0_28px_-10px_rgba(99,102,241,0.55)]" : "border-white/12")}>
            ...
            <div className="grid gap-4 pb-4 sm:grid-cols-2">
              {PLANS.map((plan) => (
                <button key={plan.id} type="button" className={cn("glass flex flex-col rounded-2xl border p-5 text-left transition-all", "hover:border-primary/40 hover:shadow-glass-sm", plan.highlight && "border-primary/35 bg-gradient-to-b from-primary/12 to-black/25 ring-1 ring-primary/25 dark:from-primary/18 dark:to-black/40", !plan.highlight && "border-white/12 bg-black/10 dark:bg-black/20")}>
                  ...
                  <span className="mt-4 inline-flex items-center gap-1 text-xs font-medium text-primary"><Check className="size-3.5" /> PortOne 연동 준비중</span>
                </button>
              ))}
            </div>
          )}
        ...
        <div className="shrink-0 border-t border-white/10 bg-background/40 px-6 py-5 backdrop-blur-sm dark:bg-black/25 sm:px-8">
          {tab === "credit" ? (
            <div className="mb-4 flex items-baseline justify-between gap-3">
              ...
              <span className="font-display text-2xl font-bold tabular-nums text-foreground">{formatWon(selectedPack.priceWon)}<span className="text-base font-semibold text-muted-foreground"> 원</span></span>
            </div>
          ) : <p className="mb-4 text-center text-[12px] text-muted-foreground">구독 플랜은 추후 오픈됩니다.</p>}
          <Button type="button" className="h-12 w-full rounded-2xl text-[15px] font-semibold" disabled={isSubmitting} onClick={() => void handlePay()}>{isSubmitting ? "처리 중…" : "결제하기"}</Button>
