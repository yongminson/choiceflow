...
- 제품 스펙(필터 유무, 소음, 유지비 등)에 대해 당신의 내부 지식(Fact)만 사용해라. 절대 없는 단점을 상상해서 지어내지 마라!
...
- 메인 승자의 "search_keyword": 유저 입력 텍스트 그대로 작성. (단, 음식 카테고리는 지역명 + 음식명)
...
export async function POST(request: Request) {
  ...
  // 🔥 멍청함의 원흉이었던 외부 검색 API(Tavily) 코드를 완전히 삭제했습니다.
