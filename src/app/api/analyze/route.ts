...
[🚨 1순위: 에러 검증 (즉각 차단 조건 - 매우 친절하게!)]
- 브랜드 불일치, 무의미한 텍스트, 동일한 사진 등 입력에 문제가 있다면 무조건 FAIL 처리해라.
- 단, "rejection_reason"에는 유저가 대화하듯 편안하게 느낄 수 있도록 "왜 에러가 났는지" 콕 찝어서 친절하게 설명해라. 
  (예: "A 옵션은 텍스트가 '김치냉장고'인데 사진은 '세탁기'가 올라왔네요! 사진을 다시 확인해 주세요.", "두 옵션에 완전히 똑같은 사진이 올라와서 비교할 수가 없어요!")
...
- 메인 승자의 "search_keyword": 유저 입력값(winnerName)을 바탕으로 하되, 오타가 있거나 너무 쓸데없이 길면(예: "모히칸 구찌 귀걸이에 반바지") 쿠팡에서 실제로 검색이 될 만한 [핵심 브랜드+상품명]으로 깔끔하게 교정/정제해라.
- 단, 유저가 특정 브랜드(예: '휴렉', '웰릭스')를 입력했다면, 검색어에 그 브랜드를 무조건 포함시켜라. 절대 '음식물 처리기'처럼 추상적인 카테고리명으로 뭉뚱그려 바꾸지 마라!
...
export async function POST(request: Request) {
  ...
  try {
    ...
    // 🔥 강제 족쇄를 풀고, AI가 뱉어낸 스마트 검색어를 1순위로 사용합니다.
    let finalSearchKeyword = compact.search_keyword || winnerName;

    // 단, 음식이면 지역 검색이 되어야 하므로 AI 검색어를 그대로 씁니다.
